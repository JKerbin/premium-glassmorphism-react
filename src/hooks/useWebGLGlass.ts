...
interface WebGLGlassConfig {
  blur: number;
  borderRadius: number;
  enableWebGL: boolean;
}

interface UseWebGLGlassReturn {
...
export const useWebGLGlass = (
  containerRef: React.RefObject<HTMLDivElement>,
  config: WebGLGlassConfig
): UseWebGLGlassReturn => {
  ...
  const render = useCallback(() => {
    ...
    gl.uniform1f(cornerRadiusUniformLocation, config.borderRadius);
    gl.uniform1f(blurRadiusUniformLocation, config.blur);
  ...
  // 初始化 WebGL
  useEffect(() => {
    if (!config.enableWebGL || !canvasRef.current) return;
  ...
  }, [config.enableWebGL, createShader, render, updateScreenshot]);

  // 当配置变化时更新截图
  useEffect(() => {
    if (!config.enableWebGL) return;
    updateScreenshot();
  }, [
    config.enableWebGL,
    config.blur,
